...
import { CREATE_ORDER_REQUEST,
  ...
  DELETE_ORDER_FAIL,
       ...
       VENDOR_ORDERS_FAIL} from '../constants/orderConstants.js'
...
export const vendorOrders = () => async(dispatch, getState) =>{
  try{
      ...
      const { data } = await axios.get("/api/v1/orders/vendor");
      dispatch({ type: VENDOR_ORDERS_SUCCESS, payload: data.orders})
  }catch(error){
      dispatch({
